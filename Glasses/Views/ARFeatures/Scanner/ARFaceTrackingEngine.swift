...
struct ARFaceTrackingEngine: UIViewRepresentable {
    // 4. The AR Engine now observes the ViewModel!
    @ObservedObject var viewModel: FaceScannerViewModel
    ...
    class Coordinator: NSObject, ARSCNViewDelegate, ARSessionDelegate {
        ...
        // 5. Look at how "dumb" this function is now. It makes NO decisions.
        func session(_ session: ARSession, didUpdate anchors: [ARAnchor]) {
            guard let faceAnchor = anchors.compactMap({ $0 as? ARFaceAnchor }).first else { return }
            
            // Just hand the anchor to the ViewModel to do the heavy lifting
